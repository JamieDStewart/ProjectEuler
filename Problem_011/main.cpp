...
{
	std::string numberBlock = std::string(
"08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08\
 49 49 99 40 17 81 18 57 60 87 17 40 98 43 69 48 04 56 62 00\
 81 49 31 73 55 79 14 29 93 71 40 67 53 88 30 03 49 13 36 65\
 52 70 95 23 04 60 11 42 69 24 68 56 01 32 56 71 37 02 36 91\
 ...
 01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48");
	...
	for (long i = 0; i < s; ++i)
	{
		...
		for (long j = 1; j < 4; ++j)
		{
			...
			long nextHorizontal = i + j;
			long nextVertical = i + (j * stride);
			long nextDiagonalDown = i + (j * stride) + j;
			long nextDiagonalUp = i - (j * stride) + j;
			...
			diagonalDown *= (nextDiagonalDown < s&& nextDiagonalDown% stride) ? numbers[nextDiagonalDown] : 0;
	...
	printf("Greatest product of 4 consecutive values in grid is: %i", result);
	return 0;
}